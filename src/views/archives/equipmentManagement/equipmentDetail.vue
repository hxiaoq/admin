...
import equipmentDetail from "@/api/archives/equipmentManagement/equipmentDetail.js";
...
export default {
  ...
  methods: {
    ...
    timeChange() {
      ...
      //导出
    exportt(data) {
        window.location.href=process.env.VUE_APP_BASE_API+"api-obdMonitor/deviceDetails/getDeviceDetailsExport?access_token="+this.token+"&cofactoryde="+this.tableQuery.factory+"&code="+this.tableQuery.code+"&startDate="+this.tableQuery.startDate+"&endDate="+this.tableQuery.endDate+"&opsId="+this.tableQuery.opsId+"&status="+this.tableQuery.status;
   ...
   goSBEdit(data){
     if(data){
        this.$router.push({
            path: "/archives/equipmentManagement/equipmentDetailEdit",
            query: { id: data.id},
          });
     }
