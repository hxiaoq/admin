...
import equipmentDetail from "@/api/archivesCarCompany/equipmentManagement/equipmentDetail.js";
...
export default {
  ...
  methods: {
    ...
    timeChange() {
      ...
      //导出
    exportt(data) {
        window.location.href=process.env.VUE_APP_BASE_API+"api-obdMonitor/deviceDetailsEnt/getDeviceDetailsExport?access_token="+this.token+"&cofactoryde="+this.tableQuery.factory+"&code="+this.tableQuery.code+"&startDate="+this.tableQuery.startDate+"&endDate="+this.tableQuery.endDate+"&opsId="+this.tableQuery.opsId+"&status="+this.tableQuery.status;
    ...
    del(data){
      ...
      cellClick(row, column, cell, event){
         if (column.property === "plateNo") {
          this.$router.push({
            path: "/archivesSoHo/carArchivesInfo",
            query: { objId: row.id },
          });
      } else {
        return;
   ...
   goSBEdit(data){
     if(data){
        this.$router.push({
            path: "/archivesSoHo/equipmentManagement/equipmentDetailEdit",
            query: { id: data.id},
          });
     }
