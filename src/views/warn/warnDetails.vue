...
export default {
  ...
  methods: {
    ...
    goHistory(data) {
      this.$router.push({
        path: '/dynamicMonitor/historyData',
        query: { companyName: data.companyName, orgId: data.orgId, plateNo: data.plateNo }
      })
    },
       //导出
    exportt(data) {
       window.location.href=process.env.VUE_APP_BASE_API+"api-obdMonitor/earlyWarning/exportEarlyWarning?access_token="+this.token+
       "&alarmType="+this.formInline.alarmType+
       "&companyName="+this.formInline.companyName+
